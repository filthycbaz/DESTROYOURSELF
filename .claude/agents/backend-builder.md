...
- Auth: JWT con `jsonwebtoken`; middleware en `server/middlewares/authMiddleware.js`
...
- Tests: `mongodb-memory-server` + `supertest` (ver agente `backend-tester`)
...
7. Si un cambio en el schema de Mongoose puede romper datos existentes: reportar para que se cree un ADR antes de implementar
